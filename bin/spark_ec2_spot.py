...
def launch_cluster(conn, opts, cluster_name):
    ...
    if existing_masters:
        ...
        # Launch slaves
        if opts.spot_price is not None:
            # Launch Master spot instance  with the requested price
            print ("Requesting %d Master as spot instances with price $%.3f" %
                   (1, opts.spot_price))
            master_req_ids = []    
            master_slave_reqs = conn.request_spot_instances(
                    price=opts.spot_price,
                    image_id=opts.ami,
                    launch_group="launch-group-%s" % cluster_name,
                    placement=opts.zone,
                    ...
                    placement_group=opts.placement_group,
                    user_data=user_data_content)
            master_req_ids += [req.id for req in master_slave_reqs]
            ...
            try:
                while True:
                    ...
                    for r in reqs:
                        id_to_req[r.id] = r
                    master_active_instance_ids = []
                    for i in master_req_ids:
                        print "Waiting status of requested id %s" % (i)
                        if i in id_to_req and id_to_req[i].state == "active":
                            master_active_instance_ids.append(id_to_req[i].instance_id)
                    if len(master_active_instance_ids) == 1:
                        print "One Master granted" 
                        reservations = conn.get_all_reservations(master_active_instance_ids)
                        master_nodes = []
                        for r in reservations:
                            master_nodes += r.instances
                        break
                    else:
                        print "Master not yet granted, waiting longer" 
            except:
                print "Canceling spot instance requests"
                conn.cancel_spot_instance_requests(my_req_ids)
                # Log a warning if any of these requests actually launched instances:
...
# Update Config file with new Ec2 master
