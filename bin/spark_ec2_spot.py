...
def launch_cluster(conn, opts, cluster_name):
    ...
    if existing_mains:
        ...
        # Launch subordinates
        if opts.spot_price is not None:
            # Launch Main spot instance  with the requested price
            print ("Requesting %d Main as spot instances with price $%.3f" %
                   (1, opts.spot_price))
            main_req_ids = []    
            main_subordinate_reqs = conn.request_spot_instances(
                    price=opts.spot_price,
                    image_id=opts.ami,
                    launch_group="launch-group-%s" % cluster_name,
                    placement=opts.zone,
                    ...
                    placement_group=opts.placement_group,
                    user_data=user_data_content)
            main_req_ids += [req.id for req in main_subordinate_reqs]
            ...
            try:
                while True:
                    ...
                    for r in reqs:
                        id_to_req[r.id] = r
                    main_active_instance_ids = []
                    for i in main_req_ids:
                        print "Waiting status of requested id %s" % (i)
                        if i in id_to_req and id_to_req[i].state == "active":
                            main_active_instance_ids.append(id_to_req[i].instance_id)
                    if len(main_active_instance_ids) == 1:
                        print "One Main granted" 
                        reservations = conn.get_all_reservations(main_active_instance_ids)
                        main_nodes = []
                        for r in reservations:
                            main_nodes += r.instances
                        break
                    else:
                        print "Main not yet granted, waiting longer" 
            except:
                print "Canceling spot instance requests"
                conn.cancel_spot_instance_requests(my_req_ids)
                # Log a warning if any of these requests actually launched instances:
...
def get_existing_cluster(conn, opts, cluster_name, die_on_error=True):
    ...
    if main_nodes != [] or not die_on_error:
        ...
        if main_nodes == [] and subordinate_nodes != []:
            print >> sys.stderr, "ERROR: Could not find main in group " + cluster_name \
                + "-main" + " in region " + opts.region
        else:
...
# Update Config file with new Ec2 main

def config_main(main_nodes):
    main = main_nodes[0].public_dns_name
    spark_main = "spark://"+main+":7077"
    hdfs_base = "hdfs://"+main+":9000/user/root/"
    os.system("cp /sar/dqserver/local.conf /sar/dqserver/local.template ")
    scmd = 'awk -F"=" \'/main/{$2="= \\"%s\\" ";print;next}1\' /sar/dqserver/local.template> /sar/dqserver/local.conf' %spark_main 
        ...
        if path.find(".svn") == -1:
            ...
            for filename in files:
                if filename[0] not in '#.~' and filename[-1] != '~' :
                    print "file name getting changed by placholder %s " % filename
...
def real_main():
    ...
    if opts.main_instance_type != "":
        if opts.main_instance_type not in EC2_INSTANCE_TYPES:
            print >> stderr, \
                "Warning: Unrecognized EC2 instance type for main-instance-type: {t}".format(
                    t=opts.main_instance_type)
           ...
           opts.main_instance_type in EC2_INSTANCE_TYPES:
            if EC2_INSTANCE_TYPES[opts.instance_type] != \
               EC2_INSTANCE_TYPES[opts.main_instance_type]:
                print >> stderr, \
                    "Error: spark-ec2 currently does not support having a main and subordinates with " + \
                    "different AMI virtualization types."
                print >> stderr, "main instance virtualization type: {t}".format(
                    t=EC2_INSTANCE_TYPES[opts.main_instance_type])
                print >> stderr, "subordinate instance virtualization type: {t}".format(
